...
const Payment = ({ deletedItem, shouldRemoveItem, setShouldRemoveItem }) => {
  ...
  return (
    <div className="container main-payment">
      ...
      {deletedItem && (
      <button className="btn btn-secondary" onClick={handleGenerateBoleto}>Gerar Boleto</button>
      )}
    </div>
  );
};
